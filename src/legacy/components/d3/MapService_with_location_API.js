...
angular.module('app').service('MapService', ['ChartDataFormat', 'TooltipService', 'mapLocationService', function(ChartDataFormat, TooltipService, mapLocationService) {
  ...
  function processLocationData(plainData, costQuery, countType) //Add accessor so we do not use count
  {
    ...
    for(var x in plainData)
    {
      ...
      var promise = mapLocationService.getLocation(x);
  ...
  return Promise.all(promises).then((res)=>{
    ...
    return Object.keys(plainData).reduce((a, e,i)=>{

        let location =  res[i].results[0].locations[0].latLng;

        columnWithUnits =  {name: e, location: {"latitude" : location.lat, "longitude" : location.lng} }

            switch(countType) {
                case "countSum":
                  // for(var y in plainData[e]) {
                    //count = plainData[e][costQuery.target][costQuery.accessor];
                    // not accumulate
                    count = plainData[e][costQuery.target][costQuery.accessor];
                  // }
  ...
  function render(scope, data, element) {
    ...
    d3.json("legacy/data/map.json", function (error, world) {
      ...
      var res =  processLocationData(scope.chartInfo.data, scope.chartInfo.query, scope.chartInfo.countType).then((res)=>{
        console.log(res);
      data = res;
